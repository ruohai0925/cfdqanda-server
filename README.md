...
- **Disabled MCP server pre-warm to free ~3 GB resident worker memory** (`worker.py:main_loop`): The MCP server (controlled-pipeline RPC backend) was unconditionally warmed up at worker boot to avoid cold-start latency on the first controlled-mode job — but then sat resident for the entire worker lifetime holding ~2-3 GB (langchain + langgraph + FAISS for Qwen3-Embedding-0.6B). With the 5 GB cgroup limit, that plus a new auto-mode Foam-Agent subprocess (~3 GB at peak) blew past the cap and the kernel OOM-killed the subprocess. Real usage data — 3 of 40 DB rows and 2 of 59 historical Storage tasks were controlled-mode (~5%) — made the trade-off obviously wrong. Made the pre-warm opt-in via `MCP_PREWARM=true` (default off). Worker resident memory dropped from ~2 GB to **~67 MB** locally and from ~2.87 GB to **~70 MB** on cloud, restoring ~4.9 GB headroom inside the 5 GB cgroup. Lazy-start path via `_ensure_mcp_server()` was already there, so the first controlled-mode job pays a one-time 30-60s cold start.
- **Filed and got merged upstream as [csml-rpi/Foam-Agent#36](https://github.com/csml-rpi/Foam-Agent/pull/36) — ChatGPT Codex HTTP client 60s read timeout fix**: The codex-subscription path uses a hand-written `requests.post()` client (`src/utils.py:287`) instead of the OpenAI SDK because chatgpt.com/backend-api/codex/responses needs OAuth + custom payload fields. The hardcoded `timeout=60` was too tight for prompts where `reasoning.summary="auto"` makes gpt-5.5 spend 30-60s on the reasoning trace before emitting tokens — a 2-inlet `elbow` channel deterministically exceeded it (Tasks 559, 562 both `HTTPSConnectionPool ... Read timed out. (read timeout=60)`). 1-line change `timeout=60` → `int(os.environ.get("FOAMAGENT_HTTP_TIMEOUT", "300"))`. Merged 2026-05-24.
- **End-to-end verification of the full Foam-Agent stack with `gpt-5.5` + PR #36** — submitted all 4 default examples from `cfdqanda-client/src/data/promptExamples.js` (excluding `tandemWing` which needs a custom mesh):
  | Case | Outcome | Time | Tokens | Retries |
  |---|---|---|---|---|
  | `cavity` (icoFoam, lid-driven) | ✅ Task 557 | 91s | 1,896 | 0 |
  | `porousBlockage` (pisoFoam) | ✅ Task 558 | 167s | 2,571 | 0 |
  | `hotRoom` (buoyantFoam, the 2026-04-09 failure case) | ✅ Task 560 | 227s | 2,487 | 0 |
  | `elbow` (icoFoam, 2 inlets + pressure outlet) | ✅ Task 565 | 350s | 2,301 | 0 |

  **4/4 pass, all zero-retry, total ~2.3-2.6K tokens per case** — confirms the gpt-5.5 bump + Foam-Agent timeout fix + worker memory headroom (MCP_PREWARM=off) combine to give the platform a clean baseline.
