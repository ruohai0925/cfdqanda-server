...
[Foam-Agent](https://foam-agent.com) 的平台服务层 —— 一个基于自然语言的 CFD 仿真自动化平台。
...
- **BYOK 路由修复——三层防护**（2026-05-21 `zhuge7777@139.com` 事件根因：12 次 BYOK 全报 "Incorrect API key"，实际 key 都是好的，是我们路由错了）：(1) `api_server.py` 的 `LLMConfig` Pydantic 模型没声明 `base_url` 字段，Pydantic **静默丢弃**——前端从 2026-03-10 起就在发，但永远到不了 worker。加上 `base_url: Optional[str] = None`。 (2) `worker.py` 两处（`_build_llm_env` controlled mode + auto mode env builder）加 `PROVIDER_DEFAULT_BASE_URLS = {'qwen': '…/dashscope.aliyuncs.com/compatible-mode/v1', 'deepseek': 'https://api.deepseek.com'}` fallback——非 OpenAI provider 没传 `base_url` 时兜底，老前端/直接调 API 也覆盖。 (3) `AISimulationTab.jsx` BYOK provider 切换时**自动填充该 provider 的 isDefault model**而非清空——避免 zhuge 那种 provider=openai + model=qwen-plus 的残留组合。
- **DeepSeek V4 系列在 BYOK 里标记为"当前不可用"**（`cfdqanda-client/src/AISimulationTab.jsx`）：2026-05-23 用 cavity 把现存所有 DeepSeek 模型都试了——`deepseek-v4-pro`、`deepseek-v4-flash`，加上已下架的 alias `deepseek-chat`/`deepseek-reasoner`——全在 planner 阶段挂掉，错误统一是 HTTP 400 `"Thinking mode does not support this tool_choice"`。V4 默认开 thinking mode，而 thinking mode 拒绝 `tool_choice` 参数，但 Foam-Agent 的 `with_structured_output(CaseSummaryModel)` 必须用 tool_choice。前端 `MODEL_VERSIONS['deepseek']` 更新到 API 实际暴露的 `v4-pro`/`v4-flash` 并加 `(currently unavailable)` 标签；选中 DeepSeek 时显示黄色警告条说明原因并建议改用 OpenAI/Anthropic。问题写入 `docs/active/Foam-Agent-Comments.md` 第 7 节——上游修复需要 `LLMService` 暴露 `extra_body` 透传，以便对 DeepSeek 传 `{"thinking": false}`。
- **平台默认 Codex 模型 `gpt-5.3-codex` → `gpt-5.5`**（`worker.py` 4 处 + 测试 + `analyze_cases.py` + `api_server.py` docstring + `AISimulationTab.jsx` 选择器 + BYOK openai 列表 + UI 文案）：GPT-5.5（2026-04-23 发布）吸收了 codex 训练栈——Terminal-Bench 2.0 上 82.7% vs 77.3%、同任务 token 数减少约 40%、延迟持平。ChatGPT Plus 及以上订阅都包含。纯升级、无副作用。前端 Codex 下拉重排为：`gpt-5.5 (default, recommended)` / `gpt-5.4` / `gpt-5.4-mini (fast)` / `gpt-5.3-codex (legacy)` / `gpt-5.2`；BYOK `openai` 列表加 `gpt-5.5` 为 `isDefault`。Memory `MEMORY.md` 的 `## Platform Default Model` 段落同步更新。
- **关闭 MCP server 预热，释放 ~3 GB 常驻 worker 内存**（`worker.py:main_loop`）：MCP server（controlled-pipeline 的 RPC 后端）原本在 worker 启动时无条件 warmup，用意是避免第一个 controlled 任务冷启动慢——但它之后**整个 worker 生命周期都常驻**，占 ~2-3 GB（langchain + langgraph + Qwen3-Embedding-0.6B 的 FAISS）。在 5 GB cgroup 上限下，再叠加一个新的 auto 模式 Foam-Agent 子进程（~3 GB 峰值），总和 >5 GB 直接 cgroup OOM 砍掉子进程。实际使用数据——DB 里 controlled 模式 3/40 行、Storage 历史 2/59 个任务（约 5%）——证明这个权衡不划算。改为 `MCP_PREWARM=true` 才预热，默认关。Worker 常驻内存：本地 ~2 GB → **~67 MB**、云端 ~2.87 GB → **~70 MB**，5 GB cgroup 内恢复 ~4.9 GB 余量。`_ensure_mcp_server()` 本来就支持懒加载，只是第一个 controlled 任务多花 30-60 秒冷启动。
- **提交并 merge 上游 [csml-rpi/Foam-Agent#36](https://github.com/csml-rpi/Foam-Agent/pull/36) ——ChatGPT Codex HTTP client 60s read timeout 修复**：codex 订阅路径用手写的 `requests.post()` 客户端（`src/utils.py:287`），不是 OpenAI SDK，因为 chatgpt.com/backend-api/codex/responses 需要 OAuth + 自定义 payload 字段。硬编码的 `timeout=60` 在 `reasoning.summary="auto"` 模式下太紧——gpt-5.5 reasoning trace 经常 30-60 秒，再加上 token 生成超 60s——`elbow`（2 入口）稳定踩到（Tasks 559、562 都报 `HTTPSConnectionPool ... Read timed out. (read timeout=60)`）。1 行修改 `timeout=60` → `int(os.environ.get("FOAMAGENT_HTTP_TIMEOUT", "300"))`。2026-05-24 merge。
- **gpt-5.5 + PR #36 完整 stack 端到端验证** —— 提了 `cfdqanda-client/src/data/promptExamples.js` 里 4 个 default 例子（`tandemWing` 需自定义 mesh 跳过）：
  | Case | 结果 | 用时 | tokens | retries |
  |---|---|---|---|---|
  | `cavity`（icoFoam，lid-driven）| ✅ Task 557 | 91s | 1,896 | 0 |
  | `porousBlockage`（pisoFoam）| ✅ Task 558 | 167s | 2,571 | 0 |
  | `hotRoom`（buoyantFoam，2026-04-09 的失败案例）| ✅ Task 560 | 227s | 2,487 | 0 |
  | `elbow`（icoFoam，2 入口 + 压力出口）| ✅ Task 565 | 350s | 2,301 | 0 |

  **4/4 通过、全部零重试、单 case 2.3-2.6K tokens** —— 证明 gpt-5.5 升级 + Foam-Agent timeout 修复 + worker 内存余量（MCP_PREWARM=off）三项叠加给了平台一个干净的基线。
